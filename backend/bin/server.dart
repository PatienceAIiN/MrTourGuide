...
String _packText(Object? v) {
  ...
  if (v is List) {
    return v.map(_packText).where((s) => s.isNotEmpty).join('\n');
  }
  if (v is Map) {
    return v.entries
...
/// Parse the model's pack reply defensively: strip fences, tolerate trailing
/// prose, and if the JSON was cut off by the token cap, repair it by closing
/// the object at the last complete field instead of failing outright.
Map<String, dynamic>? _parsePackJson(String content) {
  var t = content.trim();
  final fence = RegExp(r'```(?:json)?([\s\S]*?)```').firstMatch(t);
  if (fence != null) t = fence.group(1)!.trim();
  final start = t.indexOf('{');
  if (start < 0) return null;
  t = t.substring(start);
  // Fast path: complete object.
  final end = t.lastIndexOf('}');
  if (end > 0) {
    try {
      return jsonDecode(t.substring(0, end + 1)) as Map<String, dynamic>;
    } catch (_) {}
  }
  // Truncated: cut back to the last complete "key": value pair and close.
  for (var i = t.length - 1; i > 1; i--) {
    if (t[i] != ',' && t[i] != '"' && t[i] != ']' && t[i] != '}') continue;
    final candidate = '${t.substring(0, i + 1).replaceAll(RegExp(r',\s*$'), '')}}';
    try {
...
// ── Safety survival packs ─────────────────────────────────────────────────
// Location-aware survival info (fair taxi rates, altitude/health, signal
// dead zones, local helplines) generated by Groq and cached in Postgres for
// a week — packs stay fresh without hammering the LLM.
Future<Response> _hillPack(Request request) async {
  final place = (request.url.queryParameters['place'] ?? '').trim();
  if (place.isEmpty || place.length > 80) {
  ...
  try {
    ...
    pack['name'] = _packText(pack['name']).isEmpty
        ? place
        : _packText(pack['name']);
    pack['taxi'] = _packText(pack['taxi']);
    ...
    try {
      await _db.execute(
          Sql.named('INSERT INTO hill_packs (slug, data, updated_at) '
              'VALUES (@s, @d, now()) ON CONFLICT (slug) '
              'DO UPDATE SET data=@d, updated_at=now()'),
          parameters: {'s': slug, 'd': jsonEncode(pack)});
    } catch (_) {}
...
Future<void> main() async {
  ...
  // Hill Mode survival packs cache (auto-migrates on every deploy).
  await _db.execute('CREATE TABLE IF NOT EXISTS hill_packs ('
